...
@interface TLPhotoViewController()<UIGestureRecognizerDelegate>{
...
-(void)initLayout{
    ...
    CGFloat y = topPadding + 22 + 44,height = screenSize.height - y - topPadding;
    ...
    _imageView.userInteractionEnabled = YES;//这里必须设置为YES，否则无法接受手势操作
    [self.view addSubview:_imageView];
    //添加默认图片
    UIImage *image = [UIImage imageNamed:@"img-0"];
...
-(void)addGesture{
    ...
    longPressGesture.minimumPressDuration = 0.5;//设置长按时间，默认0.5s，一般这个值不要修改
    ...
    /*添加轻扫手势*/
    //注意一个清扫手势只能控制一个方向，默认向右，通过direction进行访问控制
...
#pragma mark 显示图片名称
-(void)showPhotoName{
    NSString *title = [NSString stringWithFormat:@"img-%i.jpg",_currentIndex];
    [self setTitle:title];
}
#pragma mark 下一张图片
-(void)nextImage{
    int index = (_currentIndex + tImageCount + 1)%tImageCount;
...
#pragma mark 拖动图片
-(void)panImage:(UIPanGestureRecognizer *)gestrue{
    if(gestrue.state == UIGestureRecognizerStateChanged){
        CGPoint translation = [gestrue translationInView:self.view];//利用拖动手势的translationInview：方法取得在相对指定视图（这里是控制器根视图）的移动
